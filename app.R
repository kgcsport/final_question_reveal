...
server <- function(input, output, session) {
  ...
  output$auth_gate <- renderUI({
    if (!rv$authed) login_ui() else NULL
  })
  ...
  output$admin_round_title  <- renderText({
    ...
    admin_hdr <- eventReactive(rv$admin_pulse, {
      list(
        round_open = G$round_open,
        unlocked   = G$unlocked_units,
        sum_pledge = G$sum_pledge,
        sum_charged= G$sum_charged,
        scale      = G$scale_factor
      )
    }, ignoreInit = FALSE)

  # recompute only when admin pulse changes
  unique_submitters_df <- eventReactive(rv$admin_pulse, {
    idx <- which(G$pledges$round == G$round)
    if (!length(idx)) return(0L)
    dplyr::n_distinct(G$pledges$user_id[idx])
  }, ignoreInit = FALSE)

  output$admin_round_status <- renderText({
    s <- admin_hdr()
    n <- unique_submitters_df()
    paste(
      sprintf("Round open: %s | Questions unlocked (total): %s", s$round_open, s$unlocked),
      sprintf("Submissions this round: %d / %d students",n, nrow(CRED)-1),
      if (!is.na(s$scale)) sprintf("Scale factor this round: %.3f", s$scale) else "Scale factor: (n/a)",
      sep = "\n"
    )
  })
  ...
  round_df <- eventReactive(rv$admin_pulse, {
    idxr <- which(G$pledges$round == G$round)
    if (length(idxr)) G$pledges[idxr, , drop = FALSE]
  ...
  output$wtp_hist <- renderPlot({
    ...
    ggplot2::ggplot(df, ggplot2::aes(x = pledge)) +
      ggplot2::geom_histogram(binwidth = G$SLIDER_STEP, boundary = 0, closed = "left") +
      ggplot2::scale_x_continuous(limits = c(0, G$MAX_PER_STUDENT),
